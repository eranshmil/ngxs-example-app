// Taken from @ngxs/store branch feature/entity-base-class
//  feat: entity base with NgxsOnInit
//  juracy committed on July 10, 2018
//
// todo: remove when pr is merged: https://github.com/ngxs/store/pull/321
...
export abstract class EntityBase<T, S extends EntityStateModel<T>>
  ...
  private _addMany(state: S, entities: T[]): EntityMutation {
    ...
    for (const entity of entities) {
      didMutate =
        this._addOne(state, entity) !== EntityMutation.None || didMutate;
    }
  ...
  private _updateMany(state: S, updates: EntityUpdate<T>[]) {
    ...
    if (didMutateEntities) {
      const didMutateIds =
        updates.filter(update => this._takeNewKey(state, newKeys, update))
          .length > 0;
